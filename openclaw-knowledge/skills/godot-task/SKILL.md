...
name: godot-task
description: >-
  Execute Godot development tasks — generate scenes, write GDScript, validate
  builds, and verify visually. Use when working on Godot scene files (.tscn),
  GDScript (.gd), Godot project tasks, debugging Godot errors, or looking up
  Godot API references.
---

# Godot Task Executor

Load reference files **progressively** — read each file only when its phase begins, not upfront.

| File | Purpose | When to read |
|------|---------|--------------|
| [quirks.md](quirks.md) | Known Godot gotchas and workarounds | Before writing any code |
| [gdscript.md](gdscript.md) | GDScript syntax reference | Before writing any code |
| [scene-generation.md](scene-generation.md) | Building `.tscn` files via headless GDScript builders | Targets include `.tscn` |
| [script-generation.md](script-generation.md) | Writing runtime `.gd` scripts for node behavior | Targets include `.gd` |
| [coordination.md](coordination.md) | Ordering scene + script generation | Targets include both `.tscn` and `.gd` |
| [test-harness.md](test-harness.md) | Writing `test/test_{id}.gd` verification scripts | Before writing test harness |
| [capture.md](capture.md) | Screenshot/video capture with GPU detection | Before capturing screenshots |
| [visual-qa.md](visual-qa.md) | Automated screenshot comparison against reference | `reference.png` exists and task has visual output |
| [doc_api/_common.md](doc_api/_common.md) | Index of ~128 common Godot classes (one-line each) | Need API ref; scan to find class names |
| [doc_api/_other.md](doc_api/_other.md) | Index of ~732 remaining Godot classes | Need API ref; class isn't in `_common.md` |
| `doc_api/{ClassName}.md` | Full API reference for a single Godot class | Need API ref; look up specific class |

Bootstrap doc_api if missing: `bash .cursor/skills/godot-task/tools/ensure_doc_api.sh`
...
4. **Generate script(s)** — write `.gd` files to `openclaw-knowledge/scripts/`
...
- `Cannot infer type` — `:=` used with `instantiate()` or polymorphic math functions, see type inference rules in [quirks.md](quirks.md)
