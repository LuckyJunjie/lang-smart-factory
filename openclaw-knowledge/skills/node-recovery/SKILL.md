---
name: node-recovery
description: Node recovery skill for Smart Factory. Diagnose and recover OpenClaw node connections via SSH. Use when a node (like Newton 192.168.3.82) becomes unresponsive. Steps: SSH connect, run openclaw doctor --fix, sync config (keep node-unique fields like auth/appSecret), restart gateway.
---
...
## Node Recovery for Smart Factory (Example)

### Scenario: Newton (192.168.3.82) becomes unresponsive

#### 1. SSH Connect
```bash
sshpass -p '<password>' ssh -o StrictHostKeyChecking=no pi@192.168.3.82 "<command>"
```

#### 2. Run openclaw doctor --fix
```bash
sshpass -p '<password>' ssh -o StrictHostKeyChecking=no pi@192.168.3.82 "/home/pi/.npm-global/bin/openclaw doctor --fix"
```

#### 3. Compare and merge configs
- Get main gateway config: `cat ~/.openclaw/openclaw.json`
- Get node config: `sshpass ... pi@192.168.3.82 "cat ~/.openclaw/openclaw.json"`
- Merge using Python, keeping node-unique fields:
  - identity
  - deviceId
  - auth (auth profiles)
  - gateway.remote.url & token
  - channels.feishu (appId, appSecret)

#### 4. Restart gateway
```bash
sshpass -p '<password>' ssh -o StrictHostKeyChecking=no pi@192.168.3.82 "pkill -f openclaw-gateway; sleep 2; cd /home/pi/.openclaw && /home/pi/.npm-global/bin/openclaw gateway &"
```

#### 5. Re-pair if needed
If node still doesn't connect, generate new QR code:
```bash
sshpass -p '<password>' ssh -o StrictHostKeyChecking=no pi@192.168.3.82 "/home/pi/.npm-global/bin/openclaw qr"
```
Then scan with OpenClaw App to re-pair.

### Known Nodes
- Newton: 192.168.3.82, password: 7721150Pan
- Tesla: 192.168.3.83 (pending)
- Jarvis: 192.168.3.79 (Mac Mini)
